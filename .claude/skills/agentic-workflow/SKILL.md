...
4. [Implement](../agentic-implement/SKILL.md): run the pinned implementer session for the active profile, validate, and publish/update the draft PR.
5. [Review](../agentic-review/SKILL.md): obtain and publish a fresh review from the configured independent reviewer.
...
Hosts differ only in invocation syntax: Codex selects a skill with `$agentic-<phase>`, Claude Code with `/agentic-<phase>`; both read the same SKILL.md, and the active profile (`workflow.py profile show`) decides which implementer backend the managed launcher starts.
